...
with st.sidebar:
    ...
    route_forecast_date = st.date_input(
        "Ngày dự báo dùng để định tuyến",
        value=pd.Timestamp.today().normalize() + pd.Timedelta(days=1),
        min_value=pd.Timestamp.today().normalize() + pd.Timedelta(days=1),
        help="Mặc định là ngày mai. Sau khi có kết quả dự báo, có thể chọn lại "
             "ngày cụ thể trong khoảng ngày dự báo ở bước 2 bên dưới.",
    )
...
history_file = st.file_uploader(
    "📂 Tải Excel dữ liệu quá khứ (khuyến nghị 365 ngày)",
    type=["xlsx", "xls", "csv"],
...
if st.session_state.forecast_result is not None:
    ...
    available_dates = sorted(pd.Timestamp(d) for d in forecast_df["date"].unique())
    min_date, max_date = available_dates[0], available_dates[-1]
    # Ngày mặc định: lấy ngày đã chọn ở sidebar (route_forecast_date) nếu nó
    # nằm trong khoảng dự báo hiện có, ngược lại lấy ngày dự báo gần nhất.
    default_date = pd.Timestamp(route_forecast_date)
    ...
    selected_date_input = st.date_input(
        ...
        min_value=min_date,
        max_value=max_date,
        key="selected_forecast_date",
    )
    # Khớp về đúng ngày có trong forecast_df gần nhất (phòng trường hợp lệch giờ/timezone)
    selected_date = min(available_dates, key=lambda d: abs((d - pd.Timestamp(selected_date_input)).days))
...
if run_btn:
    ...
    if forecast_active:
        ...
        forecast_date = pd.Timestamp(
            st.session_state.get("selected_forecast_date", sorted(forecast_df_live["date"].unique())[0])
        )
        # Khớp về đúng ngày có trong forecast_df_live gần nhất
        available_dates_live = sorted(pd.Timestamp(d) for d in forecast_df_live["date"].unique())
        forecast_date = min(available_dates_live, key=lambda d: abs((d - forecast_date).days))
        demand_map = (
...
if st.session_state.get("cluster_compare"):
    ...
    with st.expander("🗺️ Bản đồ cụm & tuyến — Mô hình đề xuất (K-means++ + CVRP)", expanded=False):
        cmap = folium.Map(location=[DEPOT_LOCATION[0], DEPOT_LOCATION[1]], zoom_start=16, tiles="OpenStreetMap")
...
if gps_enabled and "gps_engine" in st.session_state:
    ...
    with col_map:
        gmap = folium.Map(location=[DEPOT_LOCATION[0], DEPOT_LOCATION[1]], zoom_start=16, tiles="OpenStreetMap")
